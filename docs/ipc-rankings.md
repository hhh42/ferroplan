...
| constraints (gate opened at 0.23, stages a+b) | 12/120 (10.0%) — engine-reject fell 100 → 70: 30 untimed-constraint rows now attempt honestly, and storage's at-end folds banked +7 (i1–i7 solve, 0.0–10.4 s solo receipts; VAL's deterministic SIGBUS on this domain is booked validation-unavailable, internal fold-oracle green). The 70 timed rows (`within`/`always-within` — `hold-during`/`hold-after` appear nowhere in the corpus) keep the named rejection pending stage c | thin field, 3 officially-scored domains (80 instances): SGPlan5 47/80 (58.8%), MIPS-XXL 13/80 (16.3%) | **still last of 3 by rate, no longer by a mile** — 10.0% against MIPS-XXL's 16.3%, up from 4.2%; what remains is the stage-c feature, now sized with receipts (0.24's centerpiece if taken) | low |
