...
| constraints (re-entered at 0.22) | 5/120 (4.2%) — mostly engine-reject at ONE enforcement gate (constraints.rs: trajectory constraints on durative domains "not yet enforced"; the parser PARSES all four rejected variants — the 0.23 scoping decode, 100 of 115 unsolved at that single site), not search failure | thin field, 3 officially-scored domains (80 instances): SGPlan5 47/80 (58.8%), MIPS-XXL 13/80 (16.3%) | **last of 3** — well behind even the weaker entrant; the honest floor of a feature gap, not a tuning gap | low |
...
| complex-preferences | cannot attempt (modal operators unparsed — a feature gap, not a tuning gap) | SGPlan5 swept all 5 domains (105 raw solves); MIPS-XXL 2nd (25) | **last of 3**, until the feature ships | high |
