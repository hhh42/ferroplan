...
def write_junit(path: Path, failing: str | None = None) -> None:
    cases = []
    for _name, module in MODULE.CATEGORIES:
        failure = "<failure message='boom'/>" if failing == module else ""
        cases.append(f"<testcase classname='tests.{module}' name='witness'>{failure}</testcase>")
...
def test_verifier_report_alive_and_sealed(tmp_path: Path) -> None:
    ...
    write_junit(junit)
    report = MODULE.build_report(junit, ["ruff", "compileall", "shell-syntax"], "3.11")
