...
benchmarks/ipc/, and writes the raw per-instance table to
benchmarks/ipc-results.md. (benchmarks/results.md is the CURATED
oracle-comparison document consumed by the docs site — never overwrite it
from here.) Reports status, plan length, metric/cost, wall-clock, and — when
the VAL validator is available — external validation of every plan.
...
def main():
    ...
    out = ["# ferroplan vendored-corpus raw results\n",
           "Generated by `benchmarks/run.py` (see `benchmarks/results.md` for the"
           " curated oracle comparison).\n",
    ...
    if not ONLY:
        open(os.path.join(ROOT, "benchmarks", "ipc-results.md"), "w").write("\n".join(out) + "\n")
        print("\nwrote benchmarks/ipc-results.md", end="")
