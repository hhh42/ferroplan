fn main() {
    ...
    eprintln!("domain ok: {} actions, types={:?}", d.actions.len(), d.types);
    eprintln!("parsing problem...");
    let p = ferroplan::parser::parse_problem(&prob).unwrap();
    eprintln!("problem ok: {} objects, {} init atoms", p.objects.len(), p.init_atoms.len());
    ...
    eprintln!("ground: {:?} -> {} ops, {} facts", t.elapsed(), task.n_ops, task.n_facts);
}
