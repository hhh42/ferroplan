...
// @ 33e87693, file varisat/src/decision/vsids.rs); `ordered-float` was
// replaced by plain `f32` comparisons — activities are never NaN.
...
//!
//! There are a few variants that differ in which variables are bumped. This follows Minisat (and
//! others) by bumping all variables in the conflict clause and all variables resolved on during
//! conflict analysis.

use crate::{config::SolverConfig, lit::Var};
...
mod tests {
    ...
    fn rescale_bump() {
        ...
        #[allow(clippy::float_cmp)]
        {
            ...
            assert_eq!(vsids.activity[2], vsids.activity[1] * 2.0);
    ...
    fn rescale_decay() {
        ...
        #[allow(clippy::float_cmp)]
        {
            assert_eq!(vsids.activity[0], 0.0);
            assert_eq!(vsids.activity[2], vsids.activity[1] * 2.0);
            assert_eq!(vsids.activity[3], vsids.activity[1] * 3.0);
        }
    }
